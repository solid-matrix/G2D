...
internal unsafe class TextureManager : IDisposable
{
    ...
    internal readonly List<Extent2> _extents = [];
    ...
    public (VkImage, VmaAllocation, Extent2) InternalCreateTextureFromRawImage(byte[] raw)
    {
        var res = ImageResult.FromMemory(raw, ColorComponents.RedGreenBlueAlpha);

        uint width = (uint)res.Width, height = (uint)res.Height, size = (uint)res.Data.Length;

        // for (int i = 0; i < res.Data.Length; i += 4)
        // {
        ...
        // Upload data
        fixed (void* pointer = res.Data)
        {
            Vma.vmaCopyMemoryToAllocation(_device.Allocator, pointer, stagingAllocation, 0, size);
        }
        ...
        return (image, imageAllocation, new Extent2(width, height));
    }
